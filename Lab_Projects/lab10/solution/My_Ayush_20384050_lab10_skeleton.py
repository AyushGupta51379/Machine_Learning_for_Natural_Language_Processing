...
if __name__ == '__main__':
    ...
    encoder_input_train, decoder_input_train, decoder_target_train, \
        encoder_input_valid, decoder_input_valid, decoder_target_valid, vocabulary = load_data(
            'dialogue') # 'dialogue' instead of 'translation' can be done
    ...
    for i in range(0, num_encoder_layer-1):
        ### YOUR CODE HERE ###
        encoder_output = Bidirectional(GRU(units=hidden_dim, return_sequences=True,\
    ...
    ### YOUR CODE HERE ###
    decoder_output = GRU(units=2*hidden_dim, return_sequences=True,\
                                           unroll=True)(decoder_input_droped, \
                                                         initial_state=concatenate([encoder_last_h, \
                                                                                    encoder_last_hr], axis=1))
    ...
    for i in range(1, num_decoder_layer):
        ...
        decoder_output = GRU(units=2*hidden_dim, return_sequences=True,\
                                           unroll=True)(decoder_output_droped)
