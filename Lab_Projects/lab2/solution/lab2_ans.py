...
def q2():
	...
	#If you want to view this result visually, try the following code.
...
def q3():
	...
	# Your Code
	hyponym_synsets = []
	for synset in wn.synsets('dictionary'):   # find out all synsets which contian "dictionary"
		hyponym_synsets += synset.hyponyms()
	#print(type(hyponym_synsets[0]))
	hyponyms = []
	for hy in hyponym_synsets:
		hyponyms += hy.lemma_names() # get lemma names of all synsets which contain "dictionary"
	print(' '.join(hyponyms))
	...
	synsets = [wn.synset(sn) for sn in synset_names] # get synsets by using the name of synsets (string type)
	sims = [synsets[0].lch_similarity(sn) for sn in synsets ] # use lch_similarity measure
	print(sims)
	...
	def getkey(tup):
		return tup[1]
	syn_sims.sort(key=getkey,reverse=True) # reverse is True means sorting in descending order.
	# sorting
